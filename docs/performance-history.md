...
| 2026-09-17 | 5106215 | | Table groups cover a fixed 64MB of input instead of one compression batch | Same bytes at every `--gpu-mem` (previously the batch a budget allowed changed the groups, and the two backends' per-chunk estimates could differ). Compress kernel unchanged on all four corpora; 1GB of `/usr/lib` binaries 0.35% (`speed`), 0.62% (`balance`) and 0.63% (`ratio`) *smaller*; text within 0.043% either way; 43MB varied corpus identical (best of 5, RTX 5060 Ti, CUDA, 1.0GB VRAM used, ≤8% util) |
