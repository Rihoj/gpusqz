...
| 2026-09-19 | d65a5fd | | Format 3: a checksum on every stored byte (per chunk and per table group) | No measurable time: compress and decompress kernels within 0.5% and 1% on the 283MB corpus, enwik8 and 1GB of `/usr/lib` binaries (best of 5, RTX 5060 Ti idle). Space: 4 bytes per chunk, 0.016–0.025% of output at `speed`, ~0.001% at `ratio`, and proportionally more at very small chunk sizes (a 1KB-chunk fixture grew 65 x 4 bytes) |
| 2026-09-17 | ac2ad62 | | Lazy parse priced in bits (offset and length codes) instead of match length | Smaller at every profile for no measurable kernel time (within 1.3%, best of 5, RTX 5060 Ti idle, 1.0GB VRAM used, 0% util): 1GB of `/usr/lib` binaries 0.72% (`speed`), 1.13% (`balance`), 1.48% (`ratio`); 283MB corpus 0.22/0.26/0.40%; enwik8 0.21/0.29/0.42%; 43MB varied corpus 0.20/0.27/0.42%. The 1GB corpus of [Benchmarks](benchmarks.md) couldn't be rebuilt on this machine (its `/usr/include` now holds 43MB), so binaries stood in for varied data |
| 2026-09-17 | 27520b4 | | Table groups cover a fixed 64MB of input instead of one compression batch | Same bytes at every `--gpu-mem` (previously the batch a budget allowed changed the groups, and the two backends' per-chunk estimates could differ). Compress kernel unchanged on all four corpora; 1GB of `/usr/lib` binaries 0.35% (`speed`), 0.62% (`balance`) and 0.63% (`ratio`) *smaller*; text within 0.043% either way; 43MB varied corpus identical (best of 5, RTX 5060 Ti, CUDA, 1.0GB VRAM used, ≤8% util) |
...
| Lane-group layer specialised for subgroup size 32 (no `& 31`, ballot word 0, no lane base) in the Vulkan shaders | 3169109 | No gain on the NVIDIA Windows driver, slightly worse: 283MB `speed` decode `kbusy` 0.046s against 0.041s, enwik8 `ratio` 0.077s against 0.076s (best of 4). The driver already folds the masking |
| `lg_sync()` as `controlBarrier` with buffer-only memory semantics instead of `subgroupBarrier()` | 3169109 | No gain (0.045s / 0.076s against 0.042s / 0.076s). Dropping the *execution* barrier instead — a timing probe only, not correct — does recover 17–18%, so the cost is the lane synchronisation itself, not the memory semantics. See [GPU backends](backends.md#vulkan-speed) |
...
| A context-mixing profile on the GPU (32-lane lockstep, integer mixer, model shared per chunk) | 7009db6 | Prototyped in `study/predictive/cm_gpu.cu` and rejected on the go/no-go the [predictive modeling study](predictive-modeling-study.md#the-gpu-prototype-2026-09-19) set. Fast where the model is small (64KB chunks, 2^16 slots, 1.58MB per chunk: 26–33 MB/s) but that codes enwik8 to 0.3251 against `zstd -19`'s 0.2650; at the configuration that matches `zstd -19` (1MB, 2^22, 101MB per chunk) only ~32 chunks fit in flight and it runs at 5.6 MB/s, against 1–3 MB/s for `zstd -19` on one core — and that is before the word model, match model and arithmetic coder the prototype omits. Decode is symmetric, so it would be ~100x slower than today's decompressor |
| Second hash table keyed on 8 bytes (zstd's "dfast" long table), one position per slot, probed and inserted beside the 4-byte one at `balance` and `ratio`; the short table halved so the pair costs the memory the short one used to | 999b63b | A real ratio win on text, but it roughly doubles hash-table traffic, and match-poor data pays for that. At `ratio`: 283MB corpus 3.13% smaller for 29% of compress kernel time, enwik8 2.01% for 21%, 43MB varied 2.95% for 19%, but **1GB of `/usr/lib` binaries only 1.40% for 83%** (wall 1.60s → 2.38s). At `balance`, 0–0.9% smaller for 11–63%, and enwik8 0.07% *larger*. Variants measured: long table sized for L2 (4096 slots) 1.0% for 16–46%; inserting only every 4th position 0.8–0.9% for 25–44%; probing the long table only when the short match is under 16 bytes changed nothing, so the inserts are the cost. Worth revisiting only as an opt-in slower profile |
| Coding each TableGroup's counts against the previous group's, instead of only the previous row within the group | f801850 | The table section is only 0.05–0.18% of a file, and coding all the groups' counts as one stream (exactly what cross-group context buys) saves 0.18–2.2% of it: **0.0003–0.0011% of the file** on the 283MB corpus, enwik8 and 1GB of `/usr/lib` binaries. Not worth a cross-group dependency in the format |
